...
int XAxiDma_Setup(u16 DeviceId)
{
	...
	for (loop_x = 0; loop_x < HEIGHT; loop_x ++ ){
		 ...
		 * is enabled
		 */
		Xil_DCacheFlushRange((UINTPTR)(&TxBufferPtr[loop_x*MAX_PKT_LEN]), MAX_PKT_LEN);
		Xil_DCacheFlushRange((UINTPTR)(&RxBufferPtr[loop_x*MAX_PKT_LEN]), MAX_PKT_LEN);

		for(Index = 0; Index < Tries; Index ++) {
				// DMA MM2S DDR to FIFO
				Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) (UINTPTR)(&TxBufferPtr[loop_x*MAX_PKT_LEN]),
				...
				Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR)(&RxBufferPtr[loop_x*MAX_PKT_LEN]),
